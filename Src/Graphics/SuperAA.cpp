...
void SuperAA::Init(int width, int height, int port, bool streamingEnabled, const std::string &codec, const std::string &encoderType)
{
    ...
    if (streamingEnabled)
    {
        ...
        if (!initSuccess && (encoderType == "FFMPEG" || encoderType == "AUTO"))
        {
            printf("[Streaming] Attempting to initialize FFmpeg...\n");
            m_encoder = new FfmpegEncoder();
            if (m_encoder->Init(width, height, 60, port, codec, [](const uint8_t *data, size_t size, bool isKeyframe) {}))
            {
                initSuccess = true;
                chosenEncoder = "FFMPEG";
            }
            else
            {
                delete m_encoder;
                m_encoder = nullptr;
                printf("[Streaming] FFmpeg initialization failed.\n");
            }
        }
        ...
        else
        {
            ...
            printf("[Streaming] Failed to initialize any encoder (NVENC/AMF/FFmpeg). Streaming disabled.\n");
        }
    }
    else
    {
